...
source("E2_standard.R")

# data import
ammonium <- read.delim("E2_ammonium.txt", sep = "\t", dec = ",") # ammonium measurement
nitrite_1 <- read.delim("E2_nitrite_1.txt", sep = "\t", dec = ",") # nitrite measurement
nitrite_2 <- read.delim("E2_nitrite_2.txt", sep = "\t", dec = ",") # nitrite measurement after reduction
meta <- read.delim("E2_meta.txt", sep = "\t", dec = ",") # experimental meta data
...
for (i in 1:nrow(nitrite_mean)) {
  nitrite_mean[i,2] <- mean(c(nitrite_1[i,4], nitrite_1[i,5], nitrite_1[i,6], nitrite_1[i,7]))
...
for (i in 1:nrow(nitrate_mean)) {
  nitrate_mean[i,2] <- mean(c(nitrite_2[i,4], nitrite_2[i,5], nitrite_2[i,6], nitrite_2[i,7]))
